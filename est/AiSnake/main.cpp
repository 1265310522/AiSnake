...
void Map::DrawMap()
{
	...
	PIMAGE jpg_empty;
	PIMAGE jpg_wall;
	jpg_empty = newimage(); // 得到空白的图片
	getimage(jpg_empty,"D:\Cooder\Github\AiSnake\3.99 版本\PNG\empty.jpg");
	jpg_wall = newimage(); // 得到墙的图片
	getimage(jpg_empty, "D:\Cooder\Github\AiSnake\3.99 版本\PNG\wall.jpg");
	int i, j;
	// 打印上边界地图
	for (i = 0; i < BasicSetting::windows_width; i+10)
		putimage(0, i, jpg_wall);
	// 打印中间地图
	for (i = 0; i < BasicSetting::windows_height - 20; i+10) // 上边界和下边界之间的区域
	{
		for (j = 0; j < BasicSetting::windows_width; j+10) // 中间区域每一行的信息
		{
			if (i == 130 && j >= BasicSetting::windows_width - 290) // 第十四行五十一列开始打印 "#"
			{
				putimage(i, j, jpg_wall);
				continue;
			}
			if (j == 0 || j == BasicSetting::windows_width - 290 || j == BasicSetting::windows_width - 10) // 每一行的左右边界信息
			{
				putimage(i, j, jpg_wall);
			}
			else // 其余区域
				putimage(i, j, jpg_empty);
		}
		cout << endl;
	}
	// 打印下边界地图
	for (i = 0; i < BasicSetting::windows_width; i+10)
		putimage(400, i, jpg_wall);
}
...
void Snake::GetKbHit()
{
	char state = getch();  // 读取的键盘信息

	if (state == ' ') // 如果用户按下空格，则暂停 
	{
		while (1)
		{
			char temp = getch(); // 用于判断用户在暂停时的操作
			if (temp == ' ')
				break;
